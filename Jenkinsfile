					...
					if(getTokenRC.equals(200)) {
						...
						if(getCodeCoverageRC.equals(200)) {
							...
							for (int i = 0; i < codeCoverageJson.records.size(); ++i) {
								if(codeCoverageJson.records[i].ApexClassOrTrigger.Name != null) println(codeCoverageJson.records[i].ApexClassOrTrigger.Name + " " + codeCoverageJson.records[i].NumLinesCovered + " " + codeCoverageJson.records[i].NumLinesUncovered)
							}
						}
					}
				}
